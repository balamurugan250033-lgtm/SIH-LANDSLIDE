...
export default function RoadStatusScreen({ roads: initialRoads }) {
  ...
  const filtered = filter === 'all' ? roads : roads.filter(r => normalizeStatus(r.status) === filter);
    ...
    <div className="panel">
      <div className="panel-header">
        <h2>
          ...
          Road & Transport Status
        </h2>
        <span className="panel-badge">{filtered.length} corridors</span>
      </div>
      ...
      <div className="road-list">
        {filtered.length === 0 && (
          <div className="empty-state">
            ...
            <p>No road data available for this category.</p>
          </div>
        )}
        {filtered.map((road, i) => {
          const style = STATUS_STYLES[normalizeStatus(road.status)] || STATUS_STYLES.open;
          return (
            <div key={road.road_id || i} className={`road-card ${style.cls}`}>
              <div className="road-header">
                <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{road.road_name || road.road_id}</div>
                <span className={`road-status-badge ${style.badge}`}>{style.label}</span>
              </div>
              <div className="road-reason">{road.reason || 'No specific hazards reported on this stretch.'}</div>
